...
template <class T>
	...
	__global__ void init(curandState_t* states, double seed) { // 'states' is the state of the system in time 
	...
	__global__ void rndn(curandState_t* states, double* no) { // you can change the data type to unsigned int to give +ve numbers only
	...
	__global__ void s_init(curandState_t* s_states, double s_seed) {
	...
	__global__ void s_rndn(curandState_t* s_states, double* s_no) { // you can change the data type to unsigned int to give +ve numbers only
		int iid = blockIdx.x;// this is what gives it its dynamic nature, else we use thread
		s_no[iid] = curand(&s_states[iid]) % 10;
	}

 // ending the global class template
// to determine the size of the transition matrix, assuming we want to launch a client determined matrix size
/*void __global__ sizeOfTransMatrix(double* tcolumn_dev, double* trow_dev, double* tmatrix)
{
...
int main(void) {
	...
	double s_cpu[N]; // for status vector
	double* s_gpu; // for status vector
	double cpu_no[N]; // for random txm
	double* gpu_no; // for random txm
	...
	gpuErrchk(cudaMalloc((void**)&gpu_no, N * sizeof(double)));
	gpuErrchk(cudaMalloc((void**)&s_gpu, N * sizeof(double)));
	...
	// copy the a, b, c variables to the device if you want to add, activate the global function
	...
	gpuErrchk(cudaMemcpy(s_cpu, s_gpu, N * sizeof(double), cudaMemcpyDeviceToHost));
